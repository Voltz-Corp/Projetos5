    ...
    <div className="w-full h-full rounded-xl overflow-hidden border bg-card">
      ...
      <div className={cn("w-full", "h-[520px]") }>
        {loading ? (
          ...
          // @ts-ignore
          <MapContainer center={center} zoom={12} scrollWheelZoom={!isSelectOpen} className="w-full h-full z-[1]">
          <TileLayer
            // @ts-ignore - Leaflet TileLayer attribution prop typing issue
            ...
            {showClusters ? (
              <MarkerClusterGroup chunkedLoading disableClusteringAtZoom={16}>
                {filteredIncidents.map((inc) => (
                  ...
                  >
                    <Tooltip
                      // @ts-expect-error - Leaflet Tooltip offset prop typing issue
                      offset={[0, -12]}
                      opacity={1}
                      permanent={false}
                    >
